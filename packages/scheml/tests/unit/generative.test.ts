...
import type { GenerativeArtifactMetadata } from '../../src/artifacts';

// ---------------------------------------------------------------------------
// Helpers — minimal Zod-like schema helpers
// ---------------------------------------------------------------------------

const zodLikeMethods = {
  parse: (data: unknown) => data,
  safeParse: (data: unknown) => ({ success: true, data }),
};

function zodString() {
  return { ...zodLikeMethods, _def: { typeName: 'ZodString' } };
}

function zodEnum(values: string[]) {
  return { ...zodLikeMethods, _def: { typeName: 'ZodEnum', values } };
}
...
describe('validateGenerativeTrait', () => {
  ...
  it('throws ModelDefinitionError when z.enum() schema has no options', () => {
    const trait = makeGenerativeTrait({
      outputSchema: { ...zodLikeMethods, _def: { typeName: 'ZodEnum', values: [] } },
    });
    const fields = new Set(['plan', 'churned']);
...
describe('isGenerativeArtifact (from artifacts.ts)', () => {
  it('narrows artifact to GenerativeArtifactMetadata', async () => {
    const { isGenerativeArtifact } = await import('../../src/artifacts');
    ...
    expect(isGenerativeArtifact(artifact)).toBe(true);
    expect(isGenerativeArtifact({ traitType: 'predictive' })).toBe(false);
    expect(isGenerativeArtifact({ traitType: 'anomaly' })).toBe(false);
  });
});
