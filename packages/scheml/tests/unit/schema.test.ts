...
describe('normalizeSchemaText', () => {
  it('removes single-line comments', () => {
    const result = normalizeSchemaText('// comment\nmodel User { id String }');
    expect(result).not.toContain('//');
    expect(result).toContain('model User');
  });

  it('removes block comments', () => {
    const result = normalizeSchemaText('/* block */\nmodel User { id String }');
    expect(result).not.toContain('/*');
    expect(result).not.toContain('*/');
  });

  it('collapses whitespace', () => {
    const result = normalizeSchemaText('model   User   {  id  String  }');
    expect(result).not.toMatch(/\s{2,}/);
  });

  it('produces identical output for semantically-equivalent schemas (whitespace)', () => {
    const a = normalizeSchemaText('model User {\n  id String\n}');
    const b = normalizeSchemaText('model User {   id String   }');
    expect(a).toBe(b);
  });

  it('is invariant to field order within a model block', () => {
    const a = normalizeSchemaText('model User {\n  id String\n  name String\n  email String\n}');
    const b = normalizeSchemaText('model User {\n  email String\n  id String\n  name String\n}');
    expect(a).toBe(b);
  });
  ...
  it('is invariant to model block order within the file', () => {
    const a = normalizeSchemaText('model User { id String }\nmodel Post { id String }');
    const b = normalizeSchemaText('model Post { id String }\nmodel User { id String }');
    expect(a).toBe(b);
  });
});

describe('hashSchemaText', () => {
  it('returns a 64-character hex string (SHA256)', () => {
    const hash = hashSchemaText(SAMPLE_SCHEMA);
    expect(hash).toMatch(/^[a-f0-9]{64}$/);
  });

  it('is deterministic — same schema produces same hash', () => {
    const h1 = hashSchemaText(SAMPLE_SCHEMA);
    const h2 = hashSchemaText(SAMPLE_SCHEMA);
    expect(h1).toBe(h2);
  });

  it('is comment-insensitive — adding a comment does not change hash', () => {
    const withComment = SAMPLE_SCHEMA + '\n// trailing comment';
    expect(hashSchemaText(SAMPLE_SCHEMA)).toBe(hashSchemaText(withComment));
  });

  it('is field-order invariant — reordering fields does not change the hash', () => {
    const a = `model User {\n  id String @id\n  email String\n  plan String\n}`;
    const b = `model User {\n  plan String\n  id String @id\n  email String\n}`;
    expect(hashSchemaText(a)).toBe(hashSchemaText(b));
  });
  ...
  it('is change-sensitive — modifying a field changes the hash', () => {
    ...
    expect(hashSchemaText(SAMPLE_SCHEMA)).not.toBe(hashSchemaText(modified));
  });
