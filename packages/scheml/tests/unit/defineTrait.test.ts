...
import { resolveTraitGraph, topologicalSort, TraitGraphError } from '../../src/traitGraph';
import type {
  AnyTraitDefinition,
  PredictiveTrait,
  AnomalyTrait,
...
describe('defineTrait', () => {
  ...
  it('stores entity reference as string for Prisma adapter', () => {
    expect(churnRisk.entity).toBe('Customer');
  });
  ...
  it('stores entity as object for Drizzle adapter', () => {
    ...
    expect(t.entity).toBe(usersTable);
  });
...
describe('resolveTraitGraph', () => {
  ...
  it('throws CYCLE_DETECTED for a direct self-reference', () => {
    const selfRef = defineTrait('User', {
      type: 'anomaly',
      name: 'selfRef',
      baseline: ['x'],
      sensitivity: 'low',
  ...
  it('throws CYCLE_DETECTED for a two-node cycle', () => {
    const nodeA = defineTrait('User', {
      type: 'anomaly',
      name: 'nodeA',
      baseline: ['x'],
      sensitivity: 'low',
    });
    const nodeB = defineTrait('User', {
      type: 'anomaly',
      name: 'nodeB',
      baseline: ['x'],
      sensitivity: 'low',
  ...
  it('throws TRAIT_REFERENCED_BEFORE_DEFINITION for a null entry in the traits array', () => {
    const t = defineTrait('User', {
      type: 'anomaly',
      name: 't',
      baseline: ['x'],
      sensitivity: 'low',
      traits: [null as unknown as AnyTraitDefinition],
    });
