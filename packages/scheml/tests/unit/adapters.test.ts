...
import { PrismaSchemaReader } from '../../src/adapters/prisma';
import { ZodSchemaReader, createZodAdapter } from '../../src/adapters/zod';
import { DrizzleSchemaReader, createDrizzleAdapter } from '../../src/adapters/drizzle';
import { TypeOrmSchemaReader, TypeOrmDataExtractor, TypeOrmQueryInterceptor, createTypeOrmAdapter } from '../../src/adapters/typeorm';
import { getAdapter, registerAdapter, listAdapters } from '../../src/adapters/index';
import { TTLCache } from '../../src/cache';

type ZodFieldMock = {
  _def: {
    typeName: string;
    innerType?: ZodFieldMock;
  };
  isOptional?: () => boolean;
};
...
type ZodObjectMock = {
  _def: {
    typeName: 'ZodObject';
    shape: () => Record<string, ZodFieldMock>;
  };
};

type DrizzleColumnMock = { columnType: string; notNull: boolean };
type DrizzleTableMock = Record<PropertyKey, unknown> & {
  [key: string]: unknown;
};
type TypeOrmSchemaDataSource = NonNullable<ConstructorParameters<typeof TypeOrmSchemaReader>[0]>;
type TypeOrmExtractorDataSource = NonNullable<ConstructorParameters<typeof TypeOrmDataExtractor>[0]>;
type TypeOrmColumnMock = { propertyName: string; type: unknown; isNullable: boolean; enum?: unknown[] };
type TypeOrmEntityMetadataMock = { name: string; columns: TypeOrmColumnMock[]; target?: unknown };
type FakeTypeOrmDataSource = TypeOrmSchemaDataSource & {
  __find: ReturnType<typeof vi.fn>;
  __findOne: ReturnType<typeof vi.fn>;
  __findOneBy: ReturnType<typeof vi.fn>;
...
describe('ZodSchemaReader', () => {
  ...
  function fakeZodField(typeName: string, wraps?: string): ZodFieldMock {
    if (wraps) {
      return {
        _def: {
          typeName,
  ...
  function fakeZodObject(shape: Record<string, ZodFieldMock>): ZodObjectMock {
    return {
      _def: {
        typeName: 'ZodObject',
        shape: () => shape,
...
describe('DrizzleSchemaReader', () => {
  ...
  function fakeColumn(columnType: string, notNull: boolean): DrizzleColumnMock {
    return { columnType, notNull };
  }

  function fakeTable(columns: Record<string, DrizzleColumnMock>): DrizzleTableMock {
    // Use the Symbol key that DrizzleSchemaReader checks first
    const table: DrizzleTableMock = {};
  ...
  it('throws when a table does not expose Drizzle column metadata', async () => {
    const reader = new DrizzleSchemaReader({ User: {} as DrizzleTableMock });
    await expect(reader.readSchema('')).rejects.toThrow('Drizzle table introspection failed');
  });
});

// ---------------------------------------------------------------------------
// TypeOrmSchemaReader / TypeOrmDataExtractor
// ---------------------------------------------------------------------------

describe('TypeOrmSchemaReader', () => {
  function fakeColumn(propertyName: string, type: unknown, isNullable = false, enumValues?: unknown[]): TypeOrmColumnMock {
    return { propertyName, type, isNullable, enum: enumValues };
  }

  function fakeEntityMetadata(name: string, columns: TypeOrmColumnMock[], target?: unknown): TypeOrmEntityMetadataMock {
    return { name, columns, target };
  }

  function fakeDataSource(entityMetadatas: TypeOrmEntityMetadataMock[], rows: Record<string, unknown>[] = []): FakeTypeOrmDataSource {
    const find = vi.fn(async () => rows);
    const findOne = vi.fn(async () => rows[0] ?? null);
    const findOneBy = vi.fn(async () => rows[0] ?? null);
    const update = vi.fn(async () => undefined);
    return {
      isInitialized: true,
      entityMetadatas,
      getRepository: vi.fn(() => ({ find, findOne, findOneBy, update })),
      __find: find,
      __findOne: findOne,
      __findOneBy: findOneBy,
      __update: update,
    };
  }

  it('reads entity metadata into a SchemaGraph', async () => {
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [
        fakeColumn('id', Number),
        fakeColumn('email', 'varchar'),
        fakeColumn('active', 'boolean'),
        fakeColumn('createdAt', Date),
        fakeColumn('role', 'enum', false, ['ADMIN', 'MEMBER']),
      ]),
    ]);

    const reader = new TypeOrmSchemaReader(dataSource);
    const graph = await reader.readSchema('');
  ...
  it('marks nullable columns as nullable', async () => {
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('score', 'float', true)]),
    ]);
  ...
  it('hashModel returns deterministic hex', async () => {
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('id', Number)]),
    ]);

    const reader = new TypeOrmSchemaReader(dataSource);
    const graph = await reader.readSchema('');
    const hash = reader.hashModel(graph, 'User');
    expect(hash).toMatch(/^[0-9a-f]{64}$/);
  });

  it('extracts rows via repository.find with ordering and where filter', async () => {
    const rows = [{ id: 1, email: 'a@example.com' }];
    const target = { name: 'UserEntity' };
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('id', Number)], target),
    ], rows);

    const extractor = new TypeOrmDataExtractor(dataSource);
    const result = await extractor.extract('User', { orderBy: 'email', where: { active: true }, take: 10 });

    expect(result).toEqual(rows);
    expect(dataSource.getRepository).toHaveBeenCalledWith(target);
    expect(dataSource.__find).toHaveBeenCalledWith({
      where: { active: true },
      take: 10,
      order: { email: 'ASC' },
    });
  ...
  it('writes inference results via repository.update', async () => {
    const target = { name: 'UserEntity' };
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('id', Number)], target),
    ]);
    ...
    const extractor = new TypeOrmDataExtractor(dataSource);
    await extractor.write('User', [{ entityId: 42, prediction: 0.91 }], 'churnRisk');

    expect(dataSource.__update).toHaveBeenCalledWith({ id: 42 }, { churnRisk: 0.91 });
  });

  it('createTypeOrmAdapter produces adapter with reader and extractor', () => {
    const adapter = createTypeOrmAdapter();
    expect(adapter.name).toBe('typeorm');
    expect(typeof adapter.reader.readSchema).toBe('function');
    expect(typeof adapter.extractor?.extract).toBe('function');
    expect(typeof adapter.createInterceptor).toBe('function');
  });

  it('materializes trait properties onto rows returned by find()', async () => {
    const target = { name: 'User' };
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('id', Number)], target),
    ], [{ id: 1, churnRisk: 0.91 }]);

    const interceptor = new TypeOrmQueryInterceptor(
      { projectRoot: process.cwd(), dataSource, initializedByAdapter: false },
      [{ traitName: 'churnRisk', entityName: 'User', featureNames: ['spend'], materializedColumn: 'churnRisk' }],
      { mode: 'materialized', materializedColumnsPresent: true }
    );

    const extended = interceptor.extendClient(dataSource) as TypeOrmSchemaDataSource;
    const repository = extended.getRepository(target);
    const rows = await repository.find({ where: { active: true } });

    expect(rows[0].churnRisk).toBe(0.91);
    expect(dataSource.__find).toHaveBeenCalledWith({ where: { active: true } });
  });

  it('supports live trait injection and filtering for find()', async () => {
    const target = { name: 'User' };
    const dataSource = fakeDataSource([
      fakeEntityMetadata('User', [fakeColumn('id', Number)], target),
    ], [{ id: 1, spend: 200 }, { id: 2, spend: 50 }]);
    const predictionSession = {
      predict: vi.fn(async (_traitName: string, row: Record<string, unknown>) => ({
        modelName: 'User',
    ...
    const interceptor = new TypeOrmQueryInterceptor(
      { projectRoot: process.cwd(), dataSource, initializedByAdapter: false },
      [{ traitName: 'churnRisk', entityName: 'User', featureNames: ['spend'], supportsLiveInference: true }],
      {
        mode: 'live',
        predictionSession,
        cache: new TTLCache<string, number | string | boolean | null>(5000),
        materializedColumnsPresent: false,
      }
    );

    const extended = interceptor.extendClient(dataSource) as TypeOrmSchemaDataSource;
    const repository = extended.getRepository(target);
    const rows = await repository.find({ where: { trait: { churnRisk: { gt: 0.75 } } } });

    expect(rows).toHaveLength(1);
    expect(rows[0].id).toBe(1);
    expect(rows[0].churnRisk).toBe(0.9);
    expect(predictionSession.predict).toHaveBeenCalledTimes(2);
  });
});
...
describe('adapter registry', () => {
  ...
  it('getAdapter("typeorm") returns a typeorm adapter', () => {
    const adapter = getAdapter('typeorm');
    expect(adapter.name).toBe('typeorm');
  });

  it('getAdapter throws for unknown adapter name', () => {
    expect(() => getAdapter('not-real')).toThrow(/unknown adapter/);
  });
